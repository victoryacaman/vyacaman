...
| ATTENDANCE PANEL | Panel de Asistencia — static shell in Supabase Storage (`site/asistencia.html`, project `bzesypxndifsycgxtpad`), dynamic actions via existing Supabase Edge Functions (`manager-api`, `attendance-action`) | live — `index.html` iframes the Storage public URL directly. Root cause of the earlier "text plain" symptom, confirmed via Supabase's own docs: **Edge Functions cannot serve HTML at all** — any `text/html` response is unconditionally rewritten to `text/plain` by the platform, regardless of headers set in code. This affected every attempt (Netlify, and the `asistencia`/`asistencia-web` functions alike) for the same reason. Fix: the static HTML page now lives in a public Storage bucket, which serves real content-types as designed; the page's own JS still calls the Edge Functions above for actual employee actions/dashboard data, which return JSON and were never affected. A one-time `setup-attendance-storage` Edge Function (verify_jwt off) copies the page from `asistencia` into Storage — safe to leave in place or delete once storage is populated. The Batcomputer doesn't proxy or store any of that system's data itself; manager login and employee actions happen entirely inside the embedded app. |
