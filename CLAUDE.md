...
| ATTENDANCE PANEL | Panel de Asistencia — existing Supabase Edge Function (`asistencia-web`, project `bzesypxndifsycgxtpad`) | live — `index.html` iframes the edge function URL directly. Server-side logs confirm it returns 200 with `Content-Type: text/html` on every request; earlier "text plain" failures traced to a Netlify/proxy layer in front of it, not the function itself — embedding the Supabase URL directly avoids that. The Batcomputer doesn't proxy or store any of that system's data itself; manager login and employee actions happen entirely inside the embedded app. |
