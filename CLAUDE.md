...
| PATROL LOG | Gmail-search proxy at the chat layer (no calendar connector at that layer); Google Calendar API via the same Supabase OAuth backend as COMM INTERCEPTS, for the dashboard widget | partial, pending one user step to go fully live — (1) at the Claude/MCP layer: still no calendar connector, Claude searches Gmail for invite/reservation/confirmation language (`subject:(invitation OR invite OR appointment OR reservation OR confirmed OR RSVP OR calendly OR zoom OR "google meet")`) and reports it as a best-effort proxy; misses anything without an emailed invite and isn't a substitute for a real feed; (2) in `index.html`: backend built, reusing the exact `gmail_oauth` table/OAuth client from COMM INTERCEPTS — a single refresh token can cover both `gmail.readonly` and `calendar.readonly` scopes from one consent grant, so `gmail-oauth-callback` needed zero changes. `patrol-summary` (new Edge Function, read-only, unauthenticated) refreshes an access token and calls the Calendar API for today's event count + the next upcoming event's title/time/location only — never the tokens or client secret. `index.html` polls it every 5 minutes via `usePatrolFeed`, renders it in the PTL module, and folds it into the briefing/`/status`/ticker lines, same as COMM INTERCEPTS. **Still needs, from the user:** enable the Google Calendar API in Google Cloud Console, add `calendar.readonly` to the OAuth consent screen's scopes (Data Access page), then click through the consent URL once more (same redirect, now requesting both scopes) so the stored refresh token actually carries calendar access — until then the panel reports "not yet authorized for this scope," which is expected. |
...
Note: both modules are also wired at the Claude/MCP layer independent of the dashboard — ask the Batcomputer directly (in this chat) for a live triage or briefing and it will pull real data. Both dashboard widgets now have their own backend too (see table rows above) — COMM INTERCEPTS is fully live; PATROL LOG's is built and deployed but reports "not yet authorized for this scope" in the dashboard until the one remaining Google Cloud Console step (enable Calendar API, add the scope, re-consent) is done.
