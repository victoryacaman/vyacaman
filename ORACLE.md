...
**Voice (TTS):** runs on Edge TTS, an unofficial wrapper around the same speech-synthesis service Microsoft Edge's own "Read Aloud" feature uses internally — free, no API key, no account, no card. ElevenLabs was tried first but its free tier turned out to have no usable path for API access (Voice Library voices are blocked via the API, Instant Voice Cloning requires a paid plan, and this account has no pre-provisioned Default voices to fall back on) — abandoned after confirming this empirically, not assumed. Edge TTS is **not an official, supported API** — it's a reverse-engineered protocol the community maintains, and could break without notice if Microsoft changes something server-side. If it ever stops working, the existing free browser `SpeechSynthesis` fallback (already wired into `oracle-speak`'s caller) keeps Oracle speaking regardless. Default voice: `en-US-JennyNeural`, configurable via `oracle_config.edge_tts_voice` without a redeploy.
